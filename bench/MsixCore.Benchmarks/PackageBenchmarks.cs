...
public class PackageBenchmarks : IDisposable
{
    ...
    private const int CrcSmallBytes = 64 * 1024;
    ...
    private string _packOutputStored = string.Empty;
    private string _packOutputOptimal = string.Empty;
    private byte[] _smallPackageBytes = [];
    private byte[] _largePackageBytes = [];
    private byte[] _crcSmallBuffer = [];
    private byte[] _crcLargeBuffer = [];
    ...
    public void GlobalSetup()
    {
        ...
        _packOutputStored = Path.Combine(_workRoot, "pack-stored.msix");
        _packOutputOptimal = Path.Combine(_workRoot, "pack-optimal.msix");
        ...
        _crcSmallBuffer = BuildCrcBuffer(CrcSmallBytes, seed: 101);
        _crcLargeBuffer = BuildCrcBuffer((int)LargePayloadBytes, seed: 102);
    ...
    [IterationSetup(Target = nameof(PackPackageStoredLarge))]
    public void SetupPackStoredLarge()
    {
        File.Delete(_packOutputStored);
    }

    [IterationSetup(Target = nameof(PackPackageOptimalLarge))]
    public void SetupPackOptimalLarge()
    {
        File.Delete(_packOutputOptimal);
    }

    [IterationCleanup(Target = nameof(PackPackageStoredLarge))]
    public void CleanupPackStoredLarge()
    {
        File.Delete(_packOutputStored);
    }

    [IterationCleanup(Target = nameof(PackPackageOptimalLarge))]
    public void CleanupPackOptimalLarge()
    {
        File.Delete(_packOutputOptimal);
    }
    ...
    [Benchmark(Description = "Pack loose directory to MSIX (large package, Stored)")]
    public PackResult PackPackageStoredLarge() =>
        MsixPackageBuilder.Build(
            _looseDir,
            _packOutputStored,
            new PackOptions { CompressionLevel = CompressionLevel.NoCompression });

    [Benchmark(Description = "Pack loose directory to MSIX (large package, Optimal/deflate)")]
    public PackResult PackPackageOptimalLarge() =>
        MsixPackageBuilder.Build(
            _looseDir,
            _packOutputOptimal,
            new PackOptions { CompressionLevel = CompressionLevel.Optimal });

    [Benchmark(Description = "CRC-32 scalar table-driven (64 KiB)")]
    public uint CrcScalar64KiB() => ComputeScalarCrc32(_crcSmallBuffer);

    [Benchmark(Description = "CRC-32 System.IO.Hashing (64 KiB)")]
    public uint CrcSystemHashing64KiB() => Crc32.HashToUInt32(_crcSmallBuffer);

    [Benchmark(Description = "CRC-32 scalar table-driven (10 MiB)")]
    public uint CrcScalar10MiB() => ComputeScalarCrc32(_crcLargeBuffer);

    [Benchmark(Description = "CRC-32 System.IO.Hashing (10 MiB)")]
    public uint CrcSystemHashing10MiB() => Crc32.HashToUInt32(_crcLargeBuffer);
    ...
    private static byte[] BuildCrcBuffer(int length, int seed)
    {
    ...
    private static uint ComputeScalarCrc32(byte[] buffer)
    {
