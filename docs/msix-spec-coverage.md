...
| CMS content type validation (`SPC_INDIRECT_DATA_CONTENT` OID `1.3.6.1.4.1.311.2.1.4`) | [Signing overview](https://learn.microsoft.com/en-us/windows/msix/package/signing-package-overview) | yes | `AppxDigestTable.Parse` | Rejects unexpected content types. |
| APPX indirect-data digest table parsing | [Package integrity enforcement](https://learn.microsoft.com/en-us/windows/msix/package/signing-package-overview) | yes | `AppxDigestTable.Parse` | Parses the `SpcIndirectDataContent` ASN.1, extracts the `DigestInfo.digest` OCTET STRING, and validates the `APPX` header, entry count (4 or 5), mandatory tags (AXPC/AXCD/AXCT/AXBM), optional AXCI, no duplicates, no unknown tags. SHA-256 only. **Validated against real SignTool output** (see `bench/signtool-validation-findings.md`): 148-byte table, tag order AXPC/AXCD/AXCT/AXBM, AXCI absent confirmed optional, digests byte-for-byte match. AXCI (5-entry table) remains synthetic-only. |
| AXCT digest verification (`[Content_Types].xml`) | [Package integrity enforcement](https://learn.microsoft.com/en-us/windows/msix/package/signing-package-overview) | yes | `AppxDigestTableVerifier.Verify` | SHA-256 of decompressed part compared with constant-time equality. |
| AXBM digest verification (`AppxBlockMap.xml`) | [Package integrity enforcement](https://learn.microsoft.com/en-us/windows/msix/package/signing-package-overview) | yes | `AppxDigestTableVerifier.VerifyFromSnapshots` | SHA-256 of decompressed part compared with constant-time equality. This is the critical binding: CMS signer → AXBM → authenticated block map → payload block hashes. Security-relevant parts (`AppxManifest.xml`, `AppxBlockMap.xml`, `[Content_Types].xml`, `AppxSignature.p7x`, and `AppxMetadata/CodeIntegrity.cat`) are read once into a fixed-set per-part cache; hashing, parsing, copying, and reported identity/capabilities use those same bytes. General payload remains streaming, so memory does not scale with payload size; the cached security parts are themselves unbounded in size, so a hostile package can still force allocation proportional to those five parts. Snapshot consistency is checked before payload hashing, which narrows but cannot close races. Guarantee ranking: **(1) container opened from a file path** — strongest on Windows, where the owned handle's share mode blocks in-place writes/truncation; rename-over does not affect the already-open handle. On POSIX, an open read-only descriptor does not prevent another process modifying or truncating that inode, so the cache guarantees internal consistency of verified/reported security metadata, not immutable backing bytes. **(2) caller-supplied `Stream`** — container length, central-directory offset/size, and a SHA-256 digest of raw central-directory bytes are compared; non-seekable/unreadable streams fail closed. This detects central-directory/entry-metadata changes, not same-length local-header or payload edits, read races, or deceptive streams. **(3) loose directory** — re-enumeration detects part-set/name drift, but existing content can change and mutations can race reads. For all tiers, a concurrent attacker can still alter uncached payload before or during its hash/read. Use `PackageExtractor.ExtractAndVerify`—and require its valid result—when extracted payload bytes must be trusted; it hashes those bytes during the same copy. Plain `Extract` performs no verification. |
| AXCI digest verification (`AppxMetadata/CodeIntegrity.cat`) | [Signing overview](https://learn.microsoft.com/en-us/windows/msix/package/signing-package-overview) | yes | `AppxDigestTableVerifier.Verify` | Verified when the AXCI tag is present; if the tag is present but the part is missing (or vice versa), reported as a failure. |
| AXPC/AXCD digest parsing (ZIP file records / central directory) | [Signing overview](https://learn.microsoft.com/en-us/windows/msix/package/signing-package-overview) | partial | `AppxDigestTable`, `AppxDigestTableVerifier` | Digest values are **parsed and surfaced** in structured results, but verification is **explicitly not performed** — the exact ZIP byte ranges are not recoverable from the public specification. Status is reported as `NotVerified`. |
...
| Catalog signature (`AppxMetadata/CodeIntegrity.cat`) | [Signing overview](https://learn.microsoft.com/en-us/windows/msix/package/signing-package-overview) | no | — | The catalog file's own PKCS#7 structure is not opened or verified (only its raw bytes are digest-checked via AXCI). |
