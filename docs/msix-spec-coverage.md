...
| AXBM digest verification (`AppxBlockMap.xml`) | [Package integrity enforcement](https://learn.microsoft.com/en-us/windows/msix/package/signing-package-overview) | yes | `AppxDigestTableVerifier.VerifyFromSnapshots` | SHA-256 of decompressed part compared with constant-time equality. This is the critical binding: CMS signer → AXBM → authenticated block map → payload block hashes. Parts are snapshotted once before verification to prevent TOCTOU re-read. Every `IOpcPackage` implementation must report whether its current backing part set still matches `PartNames`; block-map coverage fails on any reported drift. The built-in ZIP implementation uses one already-open archive and central-directory snapshot. **Container packages opened from a caller-supplied `Stream`** are only as safe as that stream; writable, shared, or custom stream content can still reduce the guarantee to best-effort. The loose-directory implementation re-enumerates with the same validation used at open and rejects detected additions, removals, invalid names, and case-insensitive collisions. Its verdict is point-in-time: existing file content can change afterward without part-set drift. A concurrently-writable directory remains outside a hard security boundary because mutation can race individual reads. Use `PackageExtractor.ExtractAndVerify`—and require its valid result—when extracted payload bytes must be trusted; it hashes the bytes during the same copy. Plain `Extract` performs no verification. |
