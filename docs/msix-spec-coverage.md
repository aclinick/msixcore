...
| AXBM digest verification (`AppxBlockMap.xml`) | [Package integrity enforcement](https://learn.microsoft.com/en-us/windows/msix/package/signing-package-overview) | yes | `AppxDigestTableVerifier.VerifyFromSnapshots` | SHA-256 of decompressed part compared with constant-time equality. This is the critical binding: CMS signer → AXBM → authenticated block map → payload block hashes. Parts are snapshotted once before verification to prevent TOCTOU re-read. **Container packages opened from a file** provide the strongest guarantee because verification reads one already-open archive stream. **Container packages opened from a caller-supplied `Stream`** are only as safe as that stream; a writable, shared, or custom stream reduces the guarantee to best-effort. **Loose directories** provide the weakest, best-effort guarantee: verification re-enumerates with the same validation used at open and rejects detected additions, removals, invalid names, and case-insensitive collisions. A concurrently-writable directory remains outside a hard security boundary because mutation can race individual reads. |
