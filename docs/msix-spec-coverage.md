...
| AXBM digest verification (`AppxBlockMap.xml`) | [Package integrity enforcement](https://learn.microsoft.com/en-us/windows/msix/package/signing-package-overview) | yes | `AppxDigestTableVerifier.VerifyFromSnapshots` | SHA-256 of decompressed part compared with constant-time equality. This is the critical binding: CMS signer → AXBM → authenticated block map → payload block hashes. Parts are snapshotted once before verification to prevent TOCTOU re-read. Every `IOpcPackage` implementation must report whether its current backing part set still matches `PartNames`; block-map coverage fails on any reported drift. Guarantee ranking: **(1) container opened from a file path** — strongest; the package owns the open read-only handle and one ZIP central-directory snapshot. **(2) container opened from a caller-supplied `Stream`** — at open and verification, the implementation locates the terminal EOCD/ZIP64 records and compares container length, central-directory offset/size, and a SHA-256 digest of the raw central-directory bytes. This detects appended/replaced central directories, injected/renamed entries, and changed entry metadata (including sizes and local-header offsets); a non-seekable or unreadable stream fails closed. It does **not** prove full stream immutability: same-length payload-byte changes that leave the central directory unchanged, mutation racing the re-read, or a deceptive custom stream remain outside this check. **(3) loose directory** — weakest; it re-enumerates with the same validation used at open and rejects detected additions, removals, invalid names, and case-insensitive collisions, but existing file content can change without part-set drift and mutations can race reads. Use `PackageExtractor.ExtractAndVerify`—and require its valid result—when extracted payload bytes must be trusted; it hashes the bytes during the same copy. Plain `Extract` performs no verification. |
