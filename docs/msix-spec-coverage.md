...
| AXBM digest verification (`AppxBlockMap.xml`) | [Package integrity enforcement](https://learn.microsoft.com/en-us/windows/msix/package/signing-package-overview) | yes | `AppxDigestTableVerifier.VerifyFromSnapshots` | SHA-256 of decompressed part compared with constant-time equality. This is the critical binding: CMS signer → AXBM → authenticated block map → payload block hashes. Parts are snapshotted once before verification to prevent TOCTOU re-read. **Container packages opened from a file** provide an unconditional guarantee (single archive stream). **Container packages opened from a caller-supplied `Stream`** are only as safe as the stream — a writable, shared, or custom stream can change underneath verification, reducing the guarantee to best-effort. **Loose directories** are best-effort: footprint bytes are cached on first read and full part-set drift is detected at verification time (additions AND removals, using the same symlink-safe case-insensitive enumeration as `DirectoryOpcPackage.Open`), but a concurrently-writable directory is not a hard security boundary — these checks narrow the TOCTOU window, they do not eliminate it. |
