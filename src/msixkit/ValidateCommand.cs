    ...
    public static int Run(IReadOnlyList<string> args, TextWriter output, TextWriter error)
    {
        if (!TryParse(args, out string? path, out bool json, out string? parseError))
        {
            CliContract.WriteError(
                ...
                "msixkit validate",
                parseError!,
                "Usage: msixkit validate <package-file-or-directory> [--json]");
        ...
        {
            CliContract.WriteError(output, error, json, "msixkit validate", ex.Message, null, CliContract.ErrorCode(ex));
