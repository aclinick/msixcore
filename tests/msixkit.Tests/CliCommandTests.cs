...
public class CliCommandTests : IDisposable
{
    ...
    {
        _root = Path.Combine(Path.GetTempPath(), "msixkit-cli-" + Guid.NewGuid().ToString("N"));
    ...
    public void Inspect_MissingPath_ReturnsOperationalError()
    {
        ...
        Assert.Equal(3, code);
        Assert.Contains("msixkit inspect", err);
    }
    ...
    public void Pack_BadArguments_ReturnUsageError(params string[] args)
    {
        ...
        Assert.Equal(2, code);
        Assert.Contains("Usage: msixkit pack", error);
    }
    ...
    [Fact]
    public void Program_UnknownVerbFormerAlias_IsRejected()
    {
        // 'makemsix' was an alias for 'pack'. It named the upstream C++ tool, not a verb, so it was
        // removed; assert it is genuinely gone rather than silently still routed.
        ...
        Assert.Equal(CliContract.ExitCodes.Usage, code);
        Assert.False(File.Exists(outputPath));
    }
    ...
    public void Bundle_BadArguments_ReturnUsageError(params string[] args)
    {
        ...
        Assert.Equal(2, code);
        Assert.Contains("Usage: msixkit bundle", error);
    }
    ...
    public void HumanErrors_WritePlainTextToStderrAndUseContractCodes(string verb)
    {
        ...
        Assert.Contains($"Usage: msixkit {verb}", usageError);
        Assert.Equal(3, operationalCode);
        Assert.Empty(operationalOutput);
        Assert.Contains($"msixkit {verb}:", operationalError);
    ...
    public void Program_HelpAndDispatchUseTheSameVerbRegistry()
    {
        ...
        foreach (string verb in helpVerbs)
        {
            ...
            Assert.Contains($"msixkit {verb}:", error.ToString());
